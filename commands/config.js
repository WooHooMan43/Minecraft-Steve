			...
			if (args[0] === "help") {
				...
				if (args[1] === "add") {
					...
					if (!properties.UserExceptions.includes(memberExcepted.user.id) && !memberExcepted.user.bot) {
						...
						message.reply(embed.setDescription(`Added user '${memberExcepted.user.tag}' as an exception.`));
					} else {
						message.reply(embed.setDescription(`User '${memberExcepted.user.tag}' is either already an exception or a bot.`));
					}
				} else if (args[1] === "remove") {
					...
					if (properties.UserExceptions.includes(memberUnexcepted.user.id) && !memberUnexcepted.user.bot) {
						...
						message.reply(embed.setDescription(`Added user '${memberUnexcepted.user.tag}' as an exception.`));
					} else {
						message.reply(embed.setDescription(`User '${memberUnexcepted.user.tag}' is not an exception or a bot.`));
					}
				} else if (args[1] === "list") {
